...
(function (serverside, global, $, _, JSON) {
  ...
  var initializeTabs = function (tabs) {
    ...
    var optionSelected = function (e) {
      var $option = $("option:selected", $(this)),
        ...
        targetIdx =
          $option.get(0).getAttribute("data-idx") || $option.attr("value"),
        $target;
    ...
    tabs.each(function () {
      ...
      $(this)
        ...
        .each(function () {
          ...
          // do not use .attr() as it sometimes unexplicably fails
          var targetIdx =
            $(this).find("option:selected").get(0).getAttribute("data-idx") ||
  ...
  jsonform.fieldTemplate = function (inner) {
    return (
      ...
      "<% }%>" +
      'class="jsonform-error-<%= keydash %> field' +
      ...
      '<div class="<% if (node.prepend) { %>fields<% } %>' +
  ...
  var inputFieldTemplate = function (type) {
    return {
      template:
        ...
        type +
        '" ' +
        'class=\'<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\'' +
  ...
  jsonform.elementTypes = {
    ...
    range: {
      template:
        '<div class="range"><input type="range" ' +
    ...
    checkbox: {
      template:
        '<div class="ui checkbox"><input type="checkbox" id="<%= id %>" ' +
        '<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %>' +
        'name="<%= node.name %>" value="1" <% if (value) {%>checked<% } %>' +
        '<%= (node.disabled? " disabled" : "")%>' +
        '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
        ' /><label><%= node.inlinetitle || "" %>' +
        "</label></div>",
      fieldtemplate: true,
      inputfield: true,
      ...
      onInsert: function (evt, node) {
        $(node.el).find(".ui.checkbox").checkbox();
      },
    },
    file: {
      template:
    ...
    "file-hosted-public": {
      ...
      onBeforeRender: function (data, node) {
        ...
        data.transloaditname =
          "_transloadit_jsonform_genericupload_public_" +
          node.ownerTree._transloadit_generic_public_index;
    ...
    select: {
      template:
        ...
        'class=\'ui dropdown<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\'' +
      ...
      inputfield: true,
      onInsert: function (evt, node) {
        $(node.el).find("select.dropdown").dropdown();
        $(node.el).find(".ui.selection.dropdown").css("width", "100%");
      },
    },
    imageselect: {
      template:
      ...
      onInsert: function (evt, node) {
        $(node.el).on("click", ".dropdown-menu a", function (evt) {
          ...
          evt.stopPropagation();
          var img =
            evt.target.nodeName.toLowerCase() === "img"
              ? $(evt.target)
              : $(evt.target).find("img");
    ...
    iconselect: {
      ...
      onInsert: function (evt, node) {
        $(node.el).on("click", ".dropdown-menu a", function (evt) {
          ...
          evt.stopPropagation();
          var i =
            evt.target.nodeName.toLowerCase() === "i"
              ? $(evt.target)
              : $(evt.target).find("i");
      ...
      onBeforeRender: function (data, node) {
        ...
        _.each(choices, function (choice, idx) {
          choiceshtml += _.template(
            ...
            title: hasOwnProperty(node.formElement.titleMap, choice)
              ? node.formElement.titleMap[choice]
              : choice,
            node: node,
          });
        });
    ...
    array: {
      ...
      onInsert: function (evt, node) {
        ...
        $("> span > a._jsonform-array-addmore", $nodeid).click(function (evt) {
          ...
          if (boundaries.maxItems >= 0) {
            if (node.children.length > boundaries.maxItems - 2) {
              $nodeid
                .find("> span > a._jsonform-array-addmore")
                .addClass("disabled");
            }
          ...
          if (
            ...
            $nodeid
              .find("a._jsonform-array-deletecurrent")
              .removeClass("disabled");
            $nodeid
              .find("> span > a._jsonform-array-deletelast")
              .removeClass("disabled");
          }
        });
        ...
        if (boundaries.minItems > 0 && curItems < boundaries.minItems) {
          for (
            var i = 0;
            i < boundaries.minItems - 1 &&
            $nodeid.find("> ul > li").length < boundaries.minItems;
            i++
          ) {
        ...
        if (
          ...
          $nodeid.find("a._jsonform-array-deletecurrent").addClass("disabled");
          $nodeid
            .find("> span > a._jsonform-array-deletelast")
            .addClass("disabled");
        }
        ...
        $($nodeid).on(
          "click",
          "a._jsonform-array-deletecurrent",
          function (evt) {
        ...
        $("> span > a._jsonform-array-deletelast", $nodeid).click(function (
          evt
        ) {
    ...
    tabarray: {
      ...
      childTemplate: function (inner) {
        return (
          '<div data-idx="<%= node.childPos %>" class="tab-pane">' +
          inner +
          "</div>"
        );
      },
      onBeforeRender: function (data, node) {
        ...
        _.each(node.children, function (child, idx) {
          ...
          tabs +=
            ...
            '"' +
            (idx === 0 ? ' class="active"' : "") +
            '><a class="draggable tab" data-toggle="tab" rel="' +
            escape(title) +
            '">' +
            escapeHTML(title) +
            "</a></li>";
      ...
      onInsert: function (evt, node) {
        ...
        var updateTabs = function (selIdx) {
          ...
          if (activateFirstTab) {
            $('> .tabbable > .nav-tabs [data-idx="0"]', $nodeid).addClass(
              "active"
            );
          }
          $('> .tabbable > .nav-tabs [data-toggle="tab"]', $nodeid)
            .eq(selIdx)
            .click();
        };
        ...
        $("> a._jsonform-array-deleteitem", $nodeid).click(function (evt) {
          ...
          if (boundaries.minItems > 0) {
            if (node.children.length < boundaries.minItems + 1) {
              $nodeid
                .find("> a._jsonform-array-deleteitem")
                .addClass("disabled");
            }
    ...
    help: {
      template:
        '<span class="help-block" style="padding-top:5px"><%= elt.helpvalue %></span>',
      fieldtemplate: true,
    },
    msg: {
      template: "<%= elt.msg %>",
    ...
    fieldset: {
      template:
        '<fieldset class="two fields jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
        '<% if (id) { %> id="<%= id %>"<% } %>' +
        ">" +
    ...
    submit: {
      template:
        '<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="ui primary button <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>',
    },
    button: {
      template:
        ' <button type="button" <% if (id) { %> id="<%= id %>" <% } %> class="ui button <%= elt.htmlClass?elt.htmlClass:"" %>"><%= node.title %></button> ',
    },
    actions: {
      template:
        '<div class="<%= elt.htmlClass?elt.htmlClass:"" %>"><%= children %></div>',
    },
    hidden: {
      template:
    ...
    tabs: {
      template:
        '<ul class="nav nav-tabs <%= elt.htmlClass?elt.htmlClass:"" %>"' +
        '<% if (elt.id) { %> id="<%= elt.id %>"<% } %>' +
      ...
      onInsert: function (evt, node) {
        $("#" + node.id + ">li.nav-item").on("click", function (e) {
          e.preventDefault();
          $(node.el)
            .find("div[data-tabset='" + node.id + "']>div.tab-pane.active")
            .each(function () {
      ...
      onBeforeRender: function (data, node) {
        ...
        if (node.options) {
          ...
          children = _.map(node.children, function (child, idx) {
            ...
            return {
              title:
                child.legend || child.title || "Option " + (child.childPos + 1),
          ...
          .on("change", function (evt) {
            ...
            if (option.next) {
              questionNode = _.find(node.parentNode.children, function (child) {
                return (
                  child.formElement && child.formElement.qid === option.next
                );
              });
  ...
  var getSchemaKey = function (schema, key) {
    ...
    if (schemaDef && schemaDef.$ref) {
      throw new Error(
        "JSONForm does not yet support schemas that use the " +
  ...
  var getInitialValue = function (
    ...
    tpldata = tpldata || {};
    tpldata.idx =
      tpldata.idx || (arrayPath ? arrayPath[arrayPath.length - 1] : 1);
    ...
    if (
      isSet(value) &&
      formElement &&
      hasOwnProperty(formElement.titleMap, value)
    ) {
      value = _.template(
   ...
   *  schema when set.
   */
  formNode.prototype.computeInitialValues = function (
    values,
    ignoreDefaultValues
  ) {
    var self = this;
    ...
    if (this.formElement) {
      ...
      if (this.formElement.id) {
        ...
        this.id =
          escapeSelector(this.ownerTree.formDesc.prefix) +
          "-elt-counter-" +
          _.uniqueId();
      } else if (
        this.parentNode &&
        ...
        function (prop) {
          if (_.isString(this.formElement[prop])) {
            if (this.formElement[prop].indexOf("{{values.") !== -1) {
              ...
              this[prop] = applyArrayPath(
                this.formElement[prop],
                this.arrayPath
              );
            }
            if (this[prop]) {
              this[prop] = _.template(
                this[prop],
                valueTemplateSettings
              )(formData);
            }
  ...
  formNode.prototype.generate = function () {
    ...
    if (
      ...
      template = this.parentNode.view.childTemplate(
        template,
        !isSet(this.parentNode.formElement.draggable)
          ? true
          : this.parentNode.formElement.draggable
      );
    }
  ...
  formNode.prototype.enhance = function () {
    ...
    if (this.formElement) {
      ...
      if (this.el) {
        ...
        if (this.onInput)
          $(this.el).bind("input", function (evt) {
  ...
  formTree.prototype.buildFromLayout = function (formElement, context) {
    ...
    if (formElement.key) {
      ...
      if (!schemaElement) {
        ...
        throw new Error(
          'The JSONForm object references the schema key "' +
            formElement.key +
      ...
      formElement.description =
        formElement.description || schemaElement.description;
      ...
      if (!formElement.id) {
        formElement.id =
          escapeSelector(this.formDesc.prefix) +
          "-elt-" +
          slugify(formElement.key);
      }
      ...
      if (!formElement.options && schemaElement["enum"]) {
        if (formElement.titleMap) {
          formElement.options = _.map(schemaElement["enum"], function (value) {
            return {
              value: value,
              title: hasOwnProperty(formElement.titleMap, value)
                ? formElement.titleMap[value]
                : value,
            };
          });
    ...
    if (!view) {
      throw new Error(
        'The JSONForm contains an element whose type is unknown: "' +
          formElement.type +
          '"'
      );
    }
    ...
    if (options.validate !== false) {
      ...
      if (typeof options.validate != "object") {
        if (global.JSONFormValidator) {
          validator = global.JSONFormValidator.createEnvironment(
  ...
  $.fn.jsonFormErrors = function (errors, options) {
    ...
    for (var i = 0; i < errors.length; i++) {
      ...
      var errormarkerclass =
        ".jsonform-error-" + escapeSelector(key.replace(/\./g, "---"));
